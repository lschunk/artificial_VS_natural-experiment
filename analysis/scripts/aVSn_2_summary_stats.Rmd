...
# Load packages
```{r Libraries}
pack_to_load <- c("tools", "openxlsx", "R.utils", "doBy")
...
### Cycle (before & after 2000 strokes)
```{r}
cycle <- summaryBy(.~ Cycle + Contact.material, 
                   data = imp_data[c("Cycle", "Contact.material",
...
### Cycle combined with contact and raw material
```{r}
cycle_material <- summaryBy(.~ Cycle + Raw.material + Contact.material, 
                            data = imp_data[c("Cycle", "Raw.material", 
                            "Contact.material", names(imp_data)[num.var])], FUN=nminmaxmeanmedsd)
str(cycle_material) 
...
## Write to XLSX
```{r}
write.xlsx(list(spot = spot, cycle= cycle, cycle_material = cycle_material), 
...
## Save R object
```{r}
saveObject(list(spot = spot, cycle = cycle, cycle_material = cycle_material), 
